...
func Day17() {
	...
	for p, ps := range patStr {
		...
		sh.height = len(ps)
		for i, s := range ps {
			sh.width = lib.Max(sh.width, len(s))
			for j, r := range s {
				if r == '#' {
	...
	canMove := func(r *rock, dir [2]int) bool {
		for _, sq := range r.sqs {
			...
			if col < 0 || col >= chamberWidth || lib.HasBit(chamber[row], col) {
				return false
			}
		}
		return true
	...
	dropRocks := func() {
		...
		for r := 0; r < numOfRocks; r++ {
			...
			for _, sq := range rk.sqs {
				...
				chamber[row] = lib.SetBit(chamber[row], col)
			}
			topRow = lib.Max(topRow, rk.pos[0]+rk.height-1)
			maxBelow = lib.Max(maxBelow, topRow-rk.pos[0])
			th[r] = topRow
