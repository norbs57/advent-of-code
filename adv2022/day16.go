...
func Day16() {
	...
	f = func(v, t int, open lib.Bits) int {
		...
		for _, w := range nonZeros {
			if !lib.HasBit(open, w) && dist[v][w] < t-1 {
				tw := t - dist[v][w] - 1
				release := tw * flows[w]
				release += f(w, tw, lib.SetBit(open, w))
				if release > best {
					best = release
				}
			}
	...
	g = func(v, t [2]int, open lib.Bits, current int) int {
		if v[0] > v[1] {
			return g(lib.SwapInt2(v), lib.SwapInt2(t), open, current)
		}
			...
			if !lib.HasBit(open, w) {
				for i := range v {
					if dist[v[i]][w] < t[i]-1 {
						...
						t1 := t
						t1[i] = tw
						release += g(v1, t1, lib.SetBit(open, w), current+release)
						if release > best {
							best = release
						}
					}
