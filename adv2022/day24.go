...
func Day24() {
	...
	push := func(q *lib.PrioQueue[[3]int], p [3]int, tgt [2]int) {
		d := lib.DistInt2([2]int{p[0], p[1]}, tgt)
		item := lib.Item[[3]int]{p, p[2] + d, 0}
		heap.Push(q, &item)
	}
	pop := func(q *lib.PrioQueue[[3]int]) [3]int {
		item := heap.Pop(q).(*lib.Item[[3]int])
		return item.Value
	}
	f := func(src [3]int, tgt [2]int) int {
		q := make(lib.PrioQueue[[3]int], 0)
		heap.Init(&q)
