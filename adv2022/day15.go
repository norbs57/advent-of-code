...
func Day15() {
	...
	fmt.Println(lib.SumIntvSizes(ivs) - bsRow)
...
func Day15ReadInput(y int) ([][2]int, []int, int) {
	...
	for sc.Scan() {
		...
		dist := lib.DistInt2(sensor, beacon)
...
func Day15Ivs(sensors [][2]int, dists []int, row, lwb, upb int) [][2]int {
	...
	for i, s := range sensors {
		dCol := dists[i] - lib.AbsInt(s[0]-row)
		if dCol > 0 {
			intv := [2]int{s[1] - dCol, s[1] + dCol}
			intv = lib.IntersectInt2(intv, cutIntv)
			if !lib.IsEmptyInt2(intv) {
